...
class Select {	//for using EventHandler
	public:
		...
		uint64_t GetSize( void ) {
			return abs(static_cast<int64_t>(EndOffset - StartOffset))+1;
...
//		void TagPaint( void );
	...
	public:
		...
		int LineCount( void )	{
			return (hex_ctrl->IsShown() ? hex_ctrl->LineCount() : text_ctrl->LineCount());
			}
		int ActiveLine( void )	{
			return (hex_ctrl->IsShown() ? hex_ctrl->ActiveLine() : text_ctrl->ActiveLine() ) ;
			}
		int GetByteCount( void ) {
			return (hex_ctrl->IsShown() ? hex_ctrl->GetByteCount() : text_ctrl->GetByteCount());
			}
		int GetLastPosition( void ) {
			return (hex_ctrl->IsShown() ? hex_ctrl->GetLastPosition() : text_ctrl->GetLastPosition());
			}
		int GetLocalHexInsertionPoint( void );
		int GetLocalInsertionPoint( void );
		wxString GetFormatedOffsetString( uint64_t offset ) {
			return offset_ctrl->GetFormatedOffsetString( offset );
			}
